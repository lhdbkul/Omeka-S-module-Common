...
/**
 * Textarea holding a yaml definition of the fields.
 *
 * The yaml is a mapping "name => (null | label | object)":
 * - a null value is a core field kept at its default;
 * - a scalar value is the label of a simple text field;
 * - an object is derivated from laminas elements construction with some changes
 *   for end user ux, so it accepts keys "label", "type", "required", "values"
 *   (the value options of select/radio/multicheckbox), "attributes" and
 *   "options" (the laminas options adapted to each element).
 */
class FieldsTextarea extends ArrayTextarea
{
    ...
    /**
     * System field names that are not user fields and must not appear in list.
     *
     * These names are handled at runtime and not configured here.
     * For exemple, the module ContactUs passes "id" for attached resource ids.
     *
     * @var array
     */
    ...
    public function setOptions($options)
    {
        ...
        if (isset($this->options['system_fields']) && is_array($this->options['system_fields'])) {
            $this->systemFields = array_values($this->options['system_fields']);
        }

        // The javascript editor targets the textarea by this class.
        $class = trim((string) $this->getAttribute('class'));
        if (strpos(" $class ", ' common-fields-textarea ') === false) {
            $this->setAttribute('class', trim($class . ' common-fields-textarea'));
        }
        ...
        $enableForm = !array_key_exists('enable_edit_form', $this->options) || $this->options['enable_edit_form'];
        $enableYaml = !array_key_exists('enable_edit_yaml', $this->options) || $this->options['enable_edit_yaml'];
        $default = ($this->options['default_display'] ?? 'form') === 'text' ? 'text' : 'form';

        return $this
            ->setAttribute('data-enable-form', $enableForm ? '1' : '0')
            ->setAttribute('data-enable-yaml', $enableYaml ? '1' : '0')
            ->setAttribute('data-enable-preview', empty($this->options['enable_preview']) ? '0' : '1')
            ->setAttribute('data-default-display', $default);
    }
    ...
    /**
     * Parse yaml textarea into Laminas element specifications keyed by name.
     *
     * Unlike parent, on a yaml error the raw string is returned unchanged,
     * which the validator detects to block the save.
     *
